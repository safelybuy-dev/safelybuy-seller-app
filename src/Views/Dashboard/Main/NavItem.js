    ...
    <div className="relative">
      ...
      <div ref={ref}>
        {hasDropdown && isComponentVisible && (
          <ul className="absolute header-dropdown mt-2 bg-white w-60 z-10 rounded-xl border-gray-100 border-2">
            {dropDownLinks.map((e) => (
              <Link key={Date.now() + Math.random()} to={e.url}>
                <li className="py-2 px-4 text-black rounded-xl">{e.text}</li>
              </Link>
            ))}
          </ul>
        )}
