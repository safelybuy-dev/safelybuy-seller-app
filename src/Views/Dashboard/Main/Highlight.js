...
import { getWallet, TOGGLE_WITHDRAWAL_MODAL } from 'actions/wallet.action';
...
export default function Highlight() {
  ...
  const handleWidrawal = () => {
    if (!Object.values(user).length && loadingUser) {
  ...
  return (
    <div className="w-full relative">
      <div className=" md:text-center  relative z-20 py-4 px-6 pb-8 text-white md:mr-0">
        ...
        <div className="flex flex-col bg-gradient-to-tl from-purple-700 via-purple-600 to-indigo-400 rounded-2xl p-8">
          <div className="flex justify-between items-center pb-4">
            ...
            <div className="">
              <button
                className="bg-[#ab79ff66] text-gray-50 py-2 px-4 rounded cursor-pointer"
                onClick={(e) => {
          ...
          <div className="py-6">
            ...
            <span className="block text-4xl tracking-widest font-semibold">
              ...
              {Number(loadingWallet ? 0 : wallet?.balance).toLocaleString()}
            </span>
          </div>
          <div className="py-5">
            <button
      ...
      <div ref={transactionRef}>{isComponentVisible && <Transactions />}</div>
    </div>
  );
}
