...
export default function ItemsPerPage({ options, selected, setSelected }) {
  ...
  const {
    ...
    setIsComponentVisible: setIsVisible,
  } = useComponentVisible(false);

  return (
    <div className="mx-2 w-48 relative md:w-fit md:mx-0">
      <button
        onClick={(e) => {
        ...
        className="relative w-full pl-3 pr-10 py-3 text-left cursor-pointer focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm md:pr-3 flex justify-between items-center">
